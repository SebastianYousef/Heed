...
import io.github.sebastianyousef.heed.capture.HeedListenerService
 ...
 * If the user skips step 3, Heed still works, but it is reduced to cancelling
...
fun OnboardingScreen(vm: InboxViewModel, onDone: () -> Unit) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Set up Heed") }) }) { padding ->
        Column(Modifier.padding(padding).padding(20.dp)) {
            when (step) {
                0 -> {
                    Text("Let Heed read your notifications", style = MaterialTheme.typography.headlineSmall)
                    Spacer(Modifier.height(12.dp))
                    Text(
                        "Everything stays on this phone. Heed needs notification access to see " +
                    ...
                    TextButton(onClick = {
                        ...
                        Text(if (HeedListenerService.isEnabled(context)) "Granted — continue" else "I've done it")
                    }
                }

                1 -> {
                    Text("Hand Heed the volume knob", style = MaterialTheme.typography.headlineSmall)
                    Spacer(Modifier.height(12.dp))
                    Text(
                        "Set your noisy apps to Silent in Android's settings — they'll still arrive, " +
                            "they just won't interrupt. From then on Heed is the only thing on your " +
                            ...
                            "Heed instead. It's also what buys the time to think: nothing has " +
                    ...
                    if (policies.isEmpty()) {
                        Text(
                            "No apps seen yet. Come back to this list in Settings once Heed has " +

...
abstract class HeedDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** Adds the per-app record of which shipped carve-outs the user has switched off. */
        val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "ALTER TABLE focus_rules ADD COLUMN disabledExceptions TEXT NOT NULL DEFAULT ''"
                )
            }
        }
        ...
        /** Adds the focus-session history. The live session itself lives in settings. */
        ...
        /**
         * Adds leaving an app out of the statistics, and saying what kind of time it is.
         *
         * Both live on the rule row because that is already the per-app record, and
         * because it means the same screen that sets a limit says what the app is for.
         */
        val MIGRATION_11_12 = object : Migration(11, 12) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "ALTER TABLE focus_rules ADD COLUMN excludedFromStats INTEGER NOT NULL DEFAULT 0"
                )
                db.execSQL(
                    "ALTER TABLE focus_rules ADD COLUMN category TEXT NOT NULL DEFAULT 'NEUTRAL'"
                )
            }
        }
